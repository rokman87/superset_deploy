...
function buildRowHierarchyFromRows(rowsArray: any[]) {
  ...
  rowsArray.forEach(r => {
    ...
    (r.values || []).forEach((val: any, i: number) => {
      const name = String(val);
      pathParts.push(name);
      const path = pathParts.join(' → ');

      if (!cur.children.has(name)) {
        cur.children.set(name, { name, path, isLeaf: false, key: undefined, children: new Map() });
      }
  ...
  const toNodes = (m: Map<string, any>) =>
    Array.from(m.values())
...
export default function SupersetPluginChartMyfirst(props: any) {
  const {
    ...
    // master totals from transformProps (based on formData.show_grand_totals)
    showGrandTotals = true,

    // IMPORTANT: these come from Superset control state (they реально меняются)
    // if they don't exist in props, fallback to transformProps-provided props
    showRowTotals: showRowTotalsFromProps,
    showColumnTotals: showColumnTotalsFromProps,

    // fallback props from transformProps (may be undefined)
    showRowTotalsProp,
  ...
  // master toggle hides everything
  ...
  const rowHierarchy = useMemo(() => {
    if (data?.rowHierarchy?.length) return data.rowHierarchy;
    if (data?.rows?.length) return buildRowHierarchyFromRows(data.rows);
    return [];
  }, [data]);
  ...
  const nodeAggMap = useMemo(() => {
    ...
    const mergeAgg = (target: NodeAgg, src: NodeAgg) => {
      Object.keys(src).forEach(colKey => {
        Object.keys(src[colKey]).forEach(metric => addToAgg(target, colKey, metric, src[colKey][metric]));
      });
    };
        ...
        <tr key={node.path} className={`row-header ${isSubtotalRow ? 'subtotal-row' : ''}`}>
          ...
          {effectiveShowRowTotals && (
            <td className="metric-value">
              {formatValue(node.isLeaf ? getNodeTotal(node) : (isSubtotalRow ? getNodeTotal(node) : null))}
            </td>
          )}
        </tr>
      );
